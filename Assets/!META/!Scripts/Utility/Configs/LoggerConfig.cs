...
{
    [SerializeField] [Tooltip("Minimum log level")]
    private LoggerUtility.LogLevel _minimumLogLevel = LoggerUtility.LogLevel.Info;

    [SerializeField] [Tooltip("Base log file name (without extension)")]
    private string _logFileName = "game_log";
    
    [SerializeField] [Tooltip("Path to save logs (default - Logs folder in persistentDataPath (For client))")]
